...
| Validation scenarios | ✅ 109 scenarios (64 functional + 45 regression in `scenarios/regression/`, `regression: true` key, recursive-glob auto-load) |
...
| Regression flywheel | ✅ `scenarios/regression/` (45 regression scenarios) + `coverage_audit.py` "Regression scenarios: N" metric + `.github/ISSUE_TEMPLATE/bug_report.md` mandatory 回归场景 field |
...
3. **Validate** — `list_validation_scenarios()` (109 scenarios: 64 functional + 45 regression) → `run_validation_scenario(scenario="system-health")`
...
the #340 wave grew it to 107 (64 functional + 43 regression); the #342 wave grew it to 108 (64 functional + 44 regression); the #332-B wave grew it to 109 (64 functional + 45 regression). The following items remain explicitly deferred:
