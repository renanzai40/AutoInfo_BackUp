...
| Validation scenarios | ✅ 111 scenarios (64 functional + 47 regression in `scenarios/regression/`, `regression: true` key, recursive-glob auto-load) |
...
| Regression flywheel | ✅ `scenarios/regression/` (47 regression scenarios) + `coverage_audit.py` "Regression scenarios: N" metric + `.github/ISSUE_TEMPLATE/bug_report.md` mandatory 回归场景 field |
...
3. **Validate** — `list_validation_scenarios()` (111 scenarios: 64 functional + 47 regression) → `run_validation_scenario(scenario="system-health")`
...
the #342 wave grew it to 108 (64 functional + 44 regression); the #332-B wave grew it to 109 (64 functional + 45 regression); the #325 wave grew it to 110 (64 functional + 46 regression); the #319 wave grew it to 111 (64 functional + 47 regression). The following items remain explicitly deferred:
