...
| Validation scenarios | ✅ 108 scenarios (64 functional + 44 regression in `scenarios/regression/`, `regression: true` key, recursive-glob auto-load) |
...
| Regression flywheel | ✅ `scenarios/regression/` (44 regression scenarios) + `coverage_audit.py` "Regression scenarios: N" metric + `.github/ISSUE_TEMPLATE/bug_report.md` mandatory 回归场景 field |
...
3. **Validate** — `list_validation_scenarios()` (108 scenarios: 64 functional + 44 regression) → `run_validation_scenario(scenario="system-health")`
