...
| Validation scenarios | ✅ 117 scenarios (65 functional + 52 regression in `scenarios/regression/`, `regression: true` key, recursive-glob auto-load) |
...
| Regression flywheel | ✅ `scenarios/regression/` (52 regression scenarios) + `coverage_audit.py` "Regression scenarios: N" metric + `.github/ISSUE_TEMPLATE/bug_report.md` mandatory 回归场景 field |
...
3. **Validate** — `list_validation_scenarios()` (117 scenarios: 65 functional + 52 regression) → `run_validation_scenario(scenario="system-health")`
