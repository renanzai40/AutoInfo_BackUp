...
| Validation scenarios | ✅ 130 scenarios (65 functional + 65 regression in `src/autoinfo/mcp/scenarios/regression/`, `regression: true` key, recursive-glob auto-load) |
...
| Regression flywheel | ✅ `src/autoinfo/mcp/scenarios/regression/` (65 regression scenarios) + `coverage_audit.py` "Regression scenarios: N" metric + `.github/ISSUE_TEMPLATE/bug_report.md` mandatory 回归场景 field |
...
3. **Validate** — `list_validation_scenarios()` (130 scenarios: 65 functional + 65 regression) → `run_validation_scenario(scenario="system-health")`
