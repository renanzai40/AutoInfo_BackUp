...
* **security:** gitleaks v8.18.4 pre-commit secret scanning + local `no-credential-url` guard (blocks `https://token@github.com` and `ghp_`/`github_pat_` in staged files) + `gitleaks-action` in CI — prevents API-key/token leaks from reaching the repo
