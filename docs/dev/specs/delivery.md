...
| **Enterprise Briefing** | `generate_report(product="enterprise-briefing")` | Enterprise differentiated report product: adds per-item **key_metrics** (metric/value/source) on top of the premium analysis fields; renders a deterministic selection-scope label (`> **Scope**: 精选 N 条详述 · selected N of M items detailed below.`) so the Executive Summary coverage claim always matches the detailed Key Findings count (#314) | Jinja2 + LLM | Markdown, HTML, JSON, PDF, Audio, Agent | D1, D2, D3 |
...
Coverage-claim consistency (#314): the enterprise-briefing Executive Summary is LLM-written, so its opening coverage sentence can drift from the rendered Key Findings count. Two safeguards: ① the shared report synthesis prompt (`_REPORT_PRODUCT_BASE_SECTIONS`) instructs the model to name exactly the number of Key Findings it details below — never a larger count; ② the template itself renders the deterministic scope label from the actual flat render context (`key_findings|length` vs `references|length`), on both paths (`_report_data_to_dict` for reports, `_normalize_digest_product_context` for digests), so even a stale summary claim is visibly scoped.
