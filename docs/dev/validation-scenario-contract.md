...
baseline): run `list_validation_scenarios()` (expect 124) then `run_validation_scenario`
...
/ 10 unconfigured**; the suite has since grown to 124 (65 functional + 59 regression; §1.8) — the baseline shape (all
...
124 scenario files in `src/autoinfo/mcp/scenarios/` (65 functional flat in `scenarios/`
+ 59 regression in `scenarios/regression/`):
...
| I6 | Validation meta-coverage | MCP `list_validation_scenarios()`; `run_validation_scenario` for 117; then `python3 scripts/coverage_audit.py` | The 124-scenario inventory JSON, per-scenario results, and the audit report showing **146/146** covered with zero MISSING | no | meta-validation |
...
2. **No-keys profile.** Record the RED baseline (§0.4): with no BYOK keys, run
   `list_validation_scenarios()` (expect 124) then `run_validation_scenario` for every
...
1. Re-run 124 scenarios with keys configured — all must report `passed` (expect 0
...
- Scenario suite: **124/124 passed** (0 failed, 0 unconfigured) with keys set.
...
- [ ] All 124 scenarios re-run GREEN with keys configured (0 failed, 0 unconfigured); `python3 scripts/coverage_audit.py` reports 146/146 with zero MISSING.
