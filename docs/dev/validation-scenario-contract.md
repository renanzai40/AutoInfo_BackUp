...
baseline): run `list_validation_scenarios()` (expect 116) then `run_validation_scenario`
...
| I6 | Validation meta-coverage | MCP `list_validation_scenarios()`; `run_validation_scenario` for 116; then `python3 scripts/coverage_audit.py` | The 116-scenario inventory JSON, per-scenario results, and the audit report showing **146/146** covered with zero MISSING | no | meta-validation |
...
2. **No-keys profile.** Record the RED baseline (§0.4): with no BYOK keys, run
   `list_validation_scenarios()` (expect 116) then `run_validation_scenario` for every
...
1. Re-run 116 scenarios with keys configured — all must report `passed` (expect 0
...
- Scenario suite: **116/116 passed** (0 failed, 0 unconfigured) with keys set.
...
- [ ] All 116 scenarios re-run GREEN with keys configured (0 failed, 0 unconfigured); `python3 scripts/coverage_audit.py` reports 146/146 with zero MISSING.
