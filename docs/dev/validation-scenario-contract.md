...
baseline): run `list_validation_scenarios()` (expect 129) then `run_validation_scenario`
...
/ 10 unconfigured**; the suite has since grown to 129 (65 functional + 64 regression; §1.8) — the baseline shape (all
...
129 scenario files in `src/autoinfo/mcp/scenarios/` (65 functional flat in `scenarios/`
+ 64 regression in `scenarios/regression/`):
...
| I6 | Validation meta-coverage | MCP `list_validation_scenarios()`; `run_validation_scenario` for 129; then `python3 scripts/coverage_audit.py` | The 129-scenario inventory JSON, per-scenario results, and the audit report showing **146/146** covered with zero MISSING | no | meta-validation |
...
2. **No-keys profile.** Record the RED baseline (§0.4): with no BYOK keys, run
   `list_validation_scenarios()` (expect 129) then `run_validation_scenario` for every
...
1. Re-run 129 scenarios with keys configured — all must report `passed` (expect 0
...
- Scenario suite: **129/129 passed** (0 failed, 0 unconfigured) with keys set.
...
- [ ] All 129 scenarios re-run GREEN with keys configured (0 failed, 0 unconfigured); `python3 scripts/coverage_audit.py` reports 146/146 with zero MISSING.
